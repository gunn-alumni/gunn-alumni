...
const ButtonGroup = ({ buttons, initialState = 0 }: ButtonGroupProps) => {
  const defaultWidthPercent = Math.round((1 / buttons.length) * 100);
  for (const i in buttons) {
    // set default props (might be a better way to do this)
    ...
    buttons[i].className
      ? null
      : (buttons[i].className = `max-w-[${defaultWidthPercent}%]`);
  }
