...
export default function Classmates () {
  ...
  const [displayContentWrapper, setDisplayContentWrapper] = useState('sm:flex-row')
  ...
  function handleSearch (inputElm) {
    ...
    if (inputElm.value != '' && inputElm.value != null && inputElm.value != undefined) {
  ...
  function showSearchResults (showRes) {
    const groupsElementsHelper = []
    const label = (showRes.length > 0) ? 'Showing Search Results Below...' : 'No Results Were Found...'
    groupsElementsHelper.push(
            <>
                <h2 key={'showResultsText'} className={'my-[0.83em] font-bold mt-[50px]'} id={'group_label'}>{label}</h2>
                <div key={'resultItem'} id={'group_content'} className="grid min-[350px]:grid-cols-2 md:flex md:flex-row gap-x-[75px] gap-y-[25px] flex-wrap overflow-x-hidden break-all">
    ...
    <>
        ...
        <div id="content_wrapper"
            ...
            <div id='siderbar_wrapper'>
                ...
                <div id='sidebar_content' className={`${mainSidebarDisplay} hidden sticky top-0 bg-[rgba(166,22,25,1)] shadow-[0_4px_30px_rgba(0,0,0,0.75)] flex flex-row`}>
                ...
                <button id='mobile_btn_sidebar' className={`${displayMobileBtnSidebar} sticky text-white bg-black hover:bg-gray-700 m-[5px] p-[10px] w-fit h-fit`}>☰ Show Sidebar</button>
